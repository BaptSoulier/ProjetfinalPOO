...
public class Game {
    ...
    private void printFinalReport(IPlayer p) {
        if (p.isDead()) {
            output.println("Vous sortez les pieds devant.\n"
                    + "Quel échec cuisant !");
        } else if (p.getLocation() != maze.exit()) {
            output.println("Vous avez abandonné la partie.\n"
                    + "Vous n'êtes qu'un couard !");
        } else if (!maze.princessIsFree()) {
            output.println("Vous êtes parti sans la princesse.\n"
                    + "Comme c'est dommage !");
        } else {
            output.println("Vous avez réussi votre mission.\n"
