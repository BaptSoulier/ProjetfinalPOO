...
final class DescriptorFactory {
    ...
    private static final String PRINCESS = "£";
    ...
    static IDescriptor createDungeon(MazeRoom r, Maze m, int i, int j) {
        return new AbstractDescriptor(r, m, i, j) {
            protected String deepVision() {
                IDungeon dj = (IDungeon) room();
                String s = null;
                if (dj.isPrincessCaptive()) {
                    s = PRINCESS;
                } else {
                    s = VISITED1;
                }
                return EMPTY1 + s + EMPTY1;
            }
        };
    }
    ...
    static IDescriptor createKey(MazeRoom r, Maze m, int i, int j) {
        return new AbstractDescriptor(r, m, i, j) {
            protected String deepVision() {
