...
import pyland.model.Door;
import pyland.model.IDoor;
import pyland.model.IDungeon;
...
public class Maze {
    ...
    /**
     * Le donjon du labyrinthe.
     */
    private IDungeon dungeon;
    private int[] dungeonPos;

    private IKeyRoom keyRoom;
    ...
    /**
     * Indique si la princesse a été libérée.
     */
    public boolean princessIsFree() {
        if (dungeon != null) {
            return !dungeon.isPrincessCaptive();
        }
        return false;
    }
    ...
    private void initRooms() {
        ...
        createAndPlaceKeyRoom();
        createAndPlaceDungeon();
    }

    /**
     * Place aléatoirement la salle contenant la clé dans le labyrinthe.
     */
    private void createAndPlaceKeyRoom() {
        Key key = new Key("une clé rouillée qui traîne là, par terre");
        final int[] tlc = new int[] {0, 0};
        final int[] brc = new int[] {rows - 1, cols - 1};
        int[] pos;
        do {
            pos = randomPlaceInRectangle(tlc, brc);
        } while (!goodPlaceForKeyRoom(pos));
        maze[pos[ROW]][pos[COL]] =
                MazeRoom.createKeyRoom(this, pos[ROW], pos[COL]);
        keyRoom = (IKeyRoom) maze[pos[ROW]][pos[COL]].getRoom();
        keyRoom.putKey(key);
    }

    private int[] randomPlaceInRectangle(int[] tlc, int[] brc) {
        int width = brc[COL] - tlc[COL] + 1;
        int height = brc[ROW] - tlc[ROW] + 1;
        int placesNb = width * height;
        int x = (int) (Math.random() * placesNb);
        int[] result = new int[] {tlc[ROW] + x / width, tlc[COL] + x % width};
        return result;
    }

    private boolean goodPlaceForKeyRoom(int[] pos) {
        IRoom r = maze[pos[ROW]][pos[COL]].getRoom();
        return r.getClass() == NormalRoom.class;
    }

    /**
     * Place aléatoirement le donjon dans le labyrinthe.
     */
    private void createAndPlaceDungeon() {
        final int n = MIN_SIZE / 2;
        final int[] tlc = new int[] {n, n};
        final int[] brc = new int[] {rows - n - 1, cols - n - 1};
        do {
            dungeonPos = randomPlaceInRectangle(tlc, brc);
        } while (!goodPlaceForDungeon(dungeonPos));
        maze[dungeonPos[ROW]][dungeonPos[COL]] =
                MazeRoom.createDungeon(this, dungeonPos[ROW], dungeonPos[COL]);
        dungeon = (IDungeon) maze[dungeonPos[ROW]][dungeonPos[COL]].getRoom();
    }

    private boolean goodPlaceForDungeon(int[] pos) {
        IRoom room = maze[pos[ROW]][pos[COL]].getRoom();
        return room.getClass() != KeyRoom.class;
    }
    ...
    private void installDungeon() {
        connect(dungeonPos);
        Key key = keyRoom.getKey();
        List around = getPlacesAround(dungeonPos);
        for (int i = 0; i < around.size(); i++) {
            Object[] tab = (Object[]) around.get(i);
            int[] crt = (int[]) tab[0];
            Direction dir = (Direction) tab[1];
            IDoor door = new Door(key);
            dig(crt, dungeonPos);
            IRoomNetwork net = RoomNetworkFactory.get();
            net.install(dungeon, dir, door);
            door.lock(key);
        }
    }

    /**
     * Retourne la liste des coordonnées et des directions (à partir du donjon)
     *  des 4 salles attenantes au donjon.
     */
    private List getPlacesAround(int[] d) {
        List result = new ArrayList(4);
        result.add(new Object[] {
                new int[] {d[ROW] - 1, d[COL]},
                Direction.NORTH});
        result.add(new Object[] {
                new int[] {d[ROW], d[COL] - 1},
                Direction.WEST});
        result.add(new Object[] {
                new int[] {d[ROW], d[COL] + 1},
                Direction.EAST});
    ...
    private List getPossibleNeighbours(int[] cell) {
        ...
        // voisine au nord
        if (cell[ROW] > 0
                && net.getRoom(r, Direction.NORTH) == null) {
            result.add(new int[] { cell[ROW] - 1, cell[COL] });
        }
        // voisine à l'est
        if (cell[COL] < cols - 1
                && net.getRoom(r, Direction.EAST) == null) {
            result.add(new int[] { cell[ROW], cell[COL] + 1 });
        }
        // voisine au sud
        if (cell[ROW] < rows - 1
                && net.getRoom(r, Direction.SOUTH) == null) {
            result.add(new int[] { cell[ROW] + 1, cell[COL] });
        }
        // voisine à l'ouest
        if (cell[COL] > 0
                && net.getRoom(r, Direction.WEST) == null) {
            result.add(new int[] { cell[ROW], cell[COL] - 1 });
        }
        return result;
    }
